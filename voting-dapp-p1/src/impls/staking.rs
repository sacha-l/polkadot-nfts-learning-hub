    ...
    T: psp22::Internal,
{
    fn stake(&mut self, amount: Balance) -> Result<(), StakingErr> {
        ...
        // the caller has some tokens already staked
        if let Some(staking_data) = staked {
            ...
            let _ = &self
                .data::<Data>()
                .staked
                .insert(&caller, &(accumulated_amount, Self::env().block_timestamp()));
        } else {
    ...
    T: Storage<Data>,
{
    fn _calculate_voting_power(&self, staking_data: &(Balance, Timestamp)) -> u128 {
        ...
        // based on 30 days in Unix time
        const UNIX_DAY: u64 = 86400;
        let month: u64 = 30 * UNIX_DAY;
        
        let decay_coefficient = time_until_unlocking / month;

        // calculate voting_power using exponential decay
        let calc_upper = 1 as u128 - (decay_coefficient as u128).pow(current_amount_staked as u32);
        let calc_lower = 1 - current_amount_staked;
        let log_result = calc_upper / calc_lower;

        let voting_power = current_amount_staked + log_result;
        return voting_power
    }
}
